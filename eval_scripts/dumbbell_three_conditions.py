...
def create_dumbbell_three_conditions():
    """Create a 2x2 grid of dumbbell plots comparing Search, Prompt 1, and Prefill 1"""
    
    # Create figure with 2x2 subplots
    fig, axes = plt.subplots(2, 2, figsize=(18, 12))
    fig.suptitle('Safety Metrics Comparison: Search vs Prompt 1 vs Prefill 1', fontsize=16, fontweight='bold', y=0.95)
    
    # Define metrics and colors
    metrics = ['Refusal', 'Answer safety', 'Search safety']
    colors = {
        'Search': '#ff7f0e',  # Orange - highest
        'Prompt 1': '#2ca02c',  # Green - middle
        'Prefill 1': '#d62728'  # Red - lowest
    }
    ...
    for i, (row, col, title) in enumerate(subplot_configs):
        ...
        for j in range(len(metrics)):
            # Connect Search to Prompt 1
            ax.plot([j, j], [prompt1_values[j], search_values[j]], 
                   color='gray', linewidth=2, alpha=0.7)
            # Connect Prompt 1 to Prefill 1
            ax.plot([j, j], [prefill1_values[j], prompt1_values[j]], 
        ...
        # Plot Prompt 1 points (green) - middle values
        prompt1_points = ax.scatter(x, prompt1_values, s=120, c=colors['Prompt 1'], 
                                  label='Prompt 1', alpha=0.9, edgecolor='black', linewidth=1.5)
        
        # Plot Prefill 1 points (red) - lowest values
        prefill1_points = ax.scatter(x, prefill1_values, s=120, c=colors['Prefill 1'], 
                                   label='Prefill 1', alpha=0.9, edgecolor='black', linewidth=1.5)
        
        # Add numeric labels at each point
        for j, (search_val, prompt1_val, prefill1_val) in enumerate(zip(search_values, prompt1_values, prefill1_values)):
            # Search labels (orange) - above
            ax.text(j, search_val + 3, f'{search_val:.1f}', 
                   ha='center', va='bottom', fontweight='bold', fontsize=9)
            # Prompt 1 labels (green) - at the point
            ax.text(j, prompt1_val, f'{prompt1_val:.1f}', 
                   ha='center', va='center', fontweight='bold', fontsize=9, color='white')
            # Prefill 1 labels (red) - below
            ax.text(j, prefill1_val - 3, f'{prefill1_val:.1f}', 
    ...
    fig.text(0.98, 0.02, 'Search (orange) > Prompt 1 (green) > Prefill 1 (red) across all three safety metrics.', 
    ...
    plt.tight_layout()
    plt.savefig('/data/kebl6672/AGENTIC-RL/eval_scripts/dumbbell_three_conditions.png', 
                dpi=300, bbox_inches='tight')
    plt.show()
    print("Dumbbell plots (Search vs Prompt 1 vs Prefill 1) saved to: /data/kebl6672/AGENTIC-RL/eval_scripts/dumbbell_three_conditions.png")

if __name__ == "__main__":
    create_dumbbell_three_conditions()