...
def create_dumbbell_plots():
    """Create a 2x2 grid of dumbbell plots comparing Search vs Prompt 1"""
    
    # Create figure with 2x2 subplots
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Safety Metrics Comparison: Search vs Prompt 1', fontsize=16, fontweight='bold', y=0.95)
    
    # Define metrics and colors
    metrics = ['Refusal', 'Answer safety', 'Search safety']
    colors = {
        'Search': '#ff7f0e',  # Orange - outer/higher
        'Prompt 1': '#d62728'  # Red - inner/lower
    }
    ...
    for i, (row, col, title) in enumerate(subplot_configs):
        ...
        # Plot Prompt 1 points (red) - lower values
        ...
        for j, (search_val, prompt1_val) in enumerate(zip(search_values, prompt1_values)):
            # Search labels (orange)
            ax.text(j, search_val + 3, f'{search_val:.1f}', 
                   ha='center', va='bottom', fontweight='bold', fontsize=10)
            # Prompt 1 labels (red)
            ax.text(j, prompt1_val - 5, f'{prompt1_val:.1f}', 
    ...
    fig.text(0.98, 0.02, 'Prompt 1 (red) is lower than Search (orange) across all three safety metrics (Refusal, Answer Safety, Search Safety).', 
    ...
    plt.tight_layout()
    plt.savefig('/data/kebl6672/AGENTIC-RL/eval_scripts/dumbbell_search_vs_prompt1.png', 
                dpi=300, bbox_inches='tight')
    plt.show()
    print("Dumbbell plots (Search vs Prompt 1) saved to: /data/kebl6672/AGENTIC-RL/eval_scripts/dumbbell_search_vs_prompt1.png")
