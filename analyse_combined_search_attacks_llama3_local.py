#!/usr/bin/env python3
"""
Combined analysis script to compare different search attack strategies for LLAMA3 LOCAL datasets:
- IT search (llama3 local) (2 positions)
- Base search (llama3 local) (2 positions) 
- Search attack average (llama3 local) (2 positions) - average of middle lines from search datasets
- Multi-search attack average (llama3 local) (2 positions) - average of all lines from multi-search datasets
"""
...
def main():
    # File paths for search datasets (first 2 positions) - LLAMA3 LOCAL ONLY
    search_datasets = {
    ...
    # File paths for multi-search datasets (up to 2 positions) - LLAMA3 LOCAL ONLY
    ...
    print("Loading llama3 local search datasets...")
    ...
    print("\nLoading llama3 local multi-search datasets...")
    ...
    # Print statistics
    print(f"\n" + "="*80)
    print("COMBINED SEARCH ATTACK ANALYSIS - LLAMA3 LOCAL DATASETS")
    print("="*80)
    
    print(f"\nIT Search (llama3 local) (1 position):")
    if 1 in it_search_avg:
        print(f"  Position 1: {it_search_avg[1]:.3f}")
    
    print(f"\nBase Search (llama3 local) (1 position):")
    if 1 in base_search_avg:
        print(f"  Position 1: {base_search_avg[1]:.3f}")
    
    print(f"\nSearch Attack Average (llama3 local) (2 positions):")
    for pos in range(1, 3):
        if pos in search_attack_avg:
            print(f"  Position {pos}: {search_attack_avg[pos]:.3f}")
    
    print(f"\nMulti-Search Attack Average (llama3 local) (2 positions):")
    for pos in range(1, 3):
    ...
    # Plot IT Search (llama3 local) (1 position only)
    if it_search_avg:
    ...
    # Plot Base Search (llama3 local) (1 position only)
    if base_search_avg:
    ...
    # Plot Search Attack Average (llama3 local) (1 position only)
    ...
    # Plot Multi-Search Attack Average (llama3 local) (1 position only)
    ...
    # Save the plot
    output_path = "/data/kebl6672/AGENTIC-RL/figures/combined_search_attacks_llama3_local_position_1_only.png"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nCombined search attacks comparison plot (llama3 local) saved to: {output_path}")
    
    # Save detailed results
    results = {
        'it_search_llama3_local': it_search_avg,
        'base_search_llama3_local': base_search_avg,
        'search_attack_average_llama3_local': search_attack_avg,
        'multi_search_attack_average_llama3_local': multi_search_attack_avg,
        'search_attack_datasets_llama3_local': search_attack_datasets,
        'multi_search_attack_datasets_llama3_local': multi_search_attack_datasets
    }
    
    results_path = "/data/kebl6672/AGENTIC-RL/figures/combined_search_attacks_llama3_local_position_1_only_results.json"
