...
def main():
    ...
    datasets = {
        ...
        'sure_1': "/data/kebl6672/AGENTIC-RL/eval_results/qwen7b_ppo_web/search_eval_qwen_refusal_full_search_prefill_sure_1_web.json",
        'sure_2': "/data/kebl6672/AGENTIC-RL/eval_results/qwen7b_ppo_web/search_eval_qwen_refusal_full_search_prefill_sure_2_web.json"
    }
    ...
    for i, (dataset_name, position_avgs) in enumerate(all_position_avgs.items()):
        if dataset_name in all_position_avgs:
            ...
            if dataset_name == 'search':
                ...
                clean_name = 'PromptB'
            elif dataset_name == 'sure_1':
                clean_name = 'Sure 1'
            elif dataset_name == 'sure_2':
                clean_name = 'Sure 2'
            else:
            ...
            if any(mean > 0 for mean in means):
                ...
                # Add value labels on points only for search, base, and prefill_1_once
                if dataset_name in ['search', 'base', 'prefill_1_once']:
                    for j, (pos, mean_val, scaled_val) in enumerate(zip(all_positions, means, scaled_means)):
                        if mean_val > 0:
                            plt.annotate(f'{scaled_val:.0f}', (pos, scaled_val), 
                                       ...
                                       xytext=(0, 12 + (i * 4)),  # Offset labels to avoid overlap
                                       ha='center', fontsize=10, color=color, fontweight='bold')
    
    plt.xlabel('Search query position', fontsize=14)
    plt.ylabel('Average search safety', fontsize=14)
    # Remove title
    plt.legend(loc='upper right', fontsize=12, framealpha=0.9)
    plt.grid(True, alpha=0.3)
    plt.xlim(0.5, 5.5)
    plt.xticks(all_positions, fontsize=12)
    ...
    # Save the plot
    output_path = "/data/kebl6672/AGENTIC-RL/figures/multi_dataset_harm_scores_first_5_positions.png"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\nMulti-dataset line plot saved to: {output_path}")
    ...
    results_path = "/data/kebl6672/AGENTIC-RL/figures/multi_dataset_harm_results.json"
