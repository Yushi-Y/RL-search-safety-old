"""
Steering under search attack: PREFILL ONLY - steer only up to the forced <search> token.

Approach:
1. Load either the Refusal or Clean Search direction extracted from IT model (harmful vs benign, NO <search>)
2. During generation, hook into target layer
3. Add coeff * d to hidden states ONLY during prefill (prompt + <search> token)
4. Do NOT steer during autoregressive generation (tokens after <search>)
5. Generate WITH search attack (prefill <search>)

Difference from steer_search.py:
- steer_search.py: steers ALL tokens (prefill + generated)
- THIS script: steers ONLY prefill tokens (prompt + <search>), not generated tokens
"""
...
DIRECTION_TYPE = "search_clean"  # Options: "refusal" or "search_clean"
...
# =============================================================================
# Steerer — hooks into layers, applies steering ONLY during prefill
# =============================================================================
...
class SearchTokenSteerer:
    ...
    Applied ONLY during prefill (seq_len > 1), NOT during autoregressive generation.
    With use_cache=True, prefill has seq_len = prompt length, generation has seq_len = 1.
    ...
    def _hook(self, module, input):
        """forward_pre_hook: modifies input to the block (resid_pre), prefill only."""
        self.call_count += 1
        ...
        # Only steer during prefill (seq_len > 1), skip during generation (seq_len == 1)
        if seq_len == 1:
            return input
        ...
        if self.call_count <= 3:
            ...
            print(f"      [DEBUG] Call {self.call_count}, seq_len={seq_len}, STEERING, orig_norm={orig_norm:.1f}, steer_norm={steer_norm:.1f} ({steer_pct:.1f}%), modified_norm={modified_norm:.1f}")
...
def main():
    ...
    print(f"\n{'='*80}\nModel: {model_name.upper()} | Direction: {DIRECTION_TYPE} | PREFILL ONLY\n{'='*80}")
    ...
    output_file = output_dir / f"steering_results_{DIRECTION_TYPE}_search_attack_prefill_only.json"
